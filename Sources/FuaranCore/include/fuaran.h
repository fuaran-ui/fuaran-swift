...
/* ------------------------------------------------------------------------- *
 * Placement verbs (Phase 833; `move` Phase 1673) — place / move / nudge /
 * duplicate / paste
 *
 ...
 * These five entry points compute the op a placement becomes, apply it, and hand
 ...
 * one document shape across all five verbs:
 *
 ...
 *
 * MOVE vs PLACE vs DUPLICATE. `move` RELOCATES a node already in the tree and it
 * KEEPS ITS ID: the emitted op is MoveNode (plus a ReorderChildren when
 * appending does not already give the wanted order), so nothing is minted and
 * nothing is remapped. That is what a drag-move is, and it is why `move` cannot
 * be spelled as place-then-remove by a caller: between those two ops the moved
 * id either does not exist or exists twice. `move` names its node with "source",
 * the same member `duplicate` uses — both name a node already in the tree — so a
 * binding writes one encoder for the pair.
 *
...
FuaranBuf fuaran_session_move(FuaranSession *session, const uint8_t *ptr,
                              size_t len);
