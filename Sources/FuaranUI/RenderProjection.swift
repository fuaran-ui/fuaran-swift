...
enum Decode {
  ...
  static func string(_ path: String, _ j: JSON) throws -> String {
    guard case .string(let s) = j else { throw wrongType(path, "JSON string") }
    return s
  }

  static func bool(_ path: String, _ j: JSON) throws -> Bool {
    guard case .bool(let b) = j else { throw wrongType(path, "JSON boolean") }
    return b
  }
  ...
  static func int(_ path: String, _ j: JSON) throws -> Int {
    guard case .number(let n) = j else { throw wrongType(path, "JSON number (integer)") }
...
extension Decode {
  ...
  static func bindingSlot(_ path: String, _ j: JSON, _ slot: StaticSlot) throws -> Binding {
    ...
    case "Filter": return .filter(name: try reqString(path, f, "name"))
    case "Selection": return .selection(nodeId: try reqString(path, f, "nodeId"))
    case "State":
      let key = try reqString(path, f, "key")
      let dv: StaticValue
      if let v = f["defaultValue"] {
  ...
  static func action(_ path: String, _ j: JSON) throws -> Action {
    ...
    case "Call":
      let endpoint = try reqString(path, f, "endpoint")
    ...
    case "Navigate": return .navigate(route: try reqString(path, f, "route"))
    case "SetState":
      return .setState(
